...
TEST_CASE("normalize_spaces") {
  REQUIRE("hithere" ==
          crypto_square::cipher("Hi there").normalize_plain_text());
}

TEST_CASE("normalize_numbers") {
  REQUIRE("123go" ==
          crypto_square::cipher("1, 2, 3 GO!").normalize_plain_text());
}
...
TEST_CASE("plain_text_9_characters") {
  ...
  const auto actual =
      crypto_square::cipher("This is fun!").plain_text_segments();

  REQUIRE(expected == actual);
}

TEST_CASE("plain_text_segments_from_phrase") {
  const std::vector<std::string> expected{"ifmanwas", "meanttos", "tayonthe",
                                          "groundgo", "dwouldha", "vegivenu",
                                          "sroots"};

  const auto actual =
      crypto_square::cipher("If man was meant to stay on the ground, god would "
                            "have given us roots.")
          .plain_text_segments();
